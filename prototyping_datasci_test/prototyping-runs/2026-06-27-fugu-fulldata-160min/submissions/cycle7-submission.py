...
BIN_SPECS = [
    ("redshift", 64), ("u_g", 40), ("g_r", 40), ("r_i", 32), ("i_z", 32),
    ("u_r", 40), ("g_i", 36), ("g_z", 36), ("r_z", 32),
    ("mag_mean", 32), ("mag_std", 24), ("mag_range", 28), ("alpha", 32), ("delta", 32),
]

TE_KEYS = [
    "spectral_type", "galaxy_population", "spec_pop",
    "redshift_qbin", "redshift_qbin_x_spec", "redshift_qbin_x_pop", "redshift_qbin_x_spec_pop",
    "u_g_qbin", "g_r_qbin", "r_i_qbin", "i_z_qbin", "u_r_qbin", "g_i_qbin", "g_z_qbin", "r_z_qbin",
    "mag_mean_qbin", "mag_std_qbin", "mag_range_qbin",
    "sky_region", "ug_gr_region", "ri_iz_region", "rz_ug_region", "rz_gr_region", "rz_gi_region", "redseq_lowz_region",
]
...
def fit_apply_target_stats(X_fit, y_fit, frames_to_transform):
    ...
    for key in TE_KEYS:
        ...
        alpha = 220.0 if ("region" in key or "_x_" in key) else 90.0
...
def make_model(seed, variant="main", final=False):
    params = dict(
    ...
    if variant == "main":
        params.update(
            n_estimators=600 if final else 470,
            learning_rate=0.048 if final else 0.055,
            num_leaves=112,
        ...
        params.update(
            n_estimators=660 if final else 500,
...
def tune_global_multipliers(proba, y_true):
    ...
    for qso_mult in np.arange(0.60, 1.701, 0.025):
        for star_mult in np.arange(0.50, 2.401, 0.025):
...
def tune_segment_multipliers(proba, y_true, segments):
    ...
    for pass_id in range(2):
        ...
        for seg, idx in enumerate(seg_indices):
            ...
            if pass_id == 0:
                q_grid = np.unique(np.r_[1.0, np.linspace(0.72, 1.42, 15)])
                s_grid = np.unique(np.r_[1.0, np.linspace(0.60, 1.95, 16)])
            else:
                q_grid = np.unique(np.clip(base_q * np.linspace(0.88, 1.14, 9), 0.50, 1.85))
                s_grid = np.unique(np.clip(base_s * np.linspace(0.85, 1.18, 10), 0.35, 2.35))
...
def main():
    ...
    if len(train_df) >= 5000 and min(pd.Series(y).value_counts()) >= 4:
        ...
        cal_model_a = make_model(SEED + 11, variant="main", final=False)
        cal_model_a.fit(X_fit, y_fit, categorical_feature=categorical_features)
        cal_proba_a = align_probabilities(cal_model_a, cal_model_a.predict_proba(X_cal)).astype("float64")

        cal_model_b = make_model(SEED + 29, variant="alt", final=False)
        cal_model_b.fit(X_fit, y_fit, categorical_feature=categorical_features)
        cal_proba_b = align_probabilities(cal_model_b, cal_model_b.predict_proba(X_cal)).astype("float64")

        cal_proba = 0.58 * cal_proba_a + 0.42 * cal_proba_b
        global_mult, seg_mult = tune_segment_multipliers(cal_proba, y_cal, calibration_segments(train_df.iloc[cal_idx]))
        del X_fit, X_cal, cal_model_a, cal_model_b, cal_proba_a, cal_proba_b, cal_proba
        gc.collect()
    else:
    ...
    final_model_a = make_model(SEED + 101, variant="main", final=True)
    final_model_a.fit(X_train, y, categorical_feature=categorical_features)
    test_proba_a = align_probabilities(final_model_a, final_model_a.predict_proba(X_test)).astype("float64")

    final_model_b = make_model(SEED + 211, variant="alt", final=True)
    final_model_b.fit(X_train, y, categorical_feature=categorical_features)
    test_proba_b = align_probabilities(final_model_b, final_model_b.predict_proba(X_test)).astype("float64")

    test_proba = 0.58 * test_proba_a + 0.42 * test_proba_b
