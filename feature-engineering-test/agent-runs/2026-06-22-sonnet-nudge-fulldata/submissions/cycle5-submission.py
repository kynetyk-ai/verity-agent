...
Improved: larger OOF for better threshold calibration, additional features,
stronger LightGBM ensemble (more estimators, diverse configs), 
and ExtraTrees diversity from sklearn.
...
def engineer_features(df, train_ref=None, class_stats=None):
    df = df.copy()
    ...
    df["sed_slope_blue"] = df["u_g"] / 1.0  # u-g per band
    df["sed_slope_red"]  = df["r_z"] / 1.0  # r-z per band
    df["sed_curvature"]  = df["u_g"] + df["i_z"] - 2 * df["g_r"]  # second derivative proxy
    ...
    df["stellar_locus"] = (df["g_r"] - 0.6 * df["r_i"])  # deviation from stellar locus
...
# OOF threshold optimization on LARGER SAMPLE for better calibration
# Use 150K rows (vs 80K before) for better representation
...
for fold_i, (tr_idx, val_idx) in enumerate(skf.split(X_samp, y_samp)):
    print(f"  OOF fold {fold_i+1}/5...")
    oof_model = make_lgbm(
        seed=fold_i * 100 + 42, n_estimators=1200, num_leaves=127, lr=0.05,
        min_child=20, subsample=0.8, colsample=0.8,
    )
