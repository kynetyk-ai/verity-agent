...
def fit_base_models(X_xgb, X_lgbm, y_idx, sample_weight, X_test_xgb, X_test_lgbm):
    ...
    for model in xgb_models:
        ...
        all_models.append((model, X_test_xgb, "xgb"))
    for model in lgbm_models:
        model.fit(X_lgbm, y_idx, sample_weight=sample_weight)
        all_models.append((model, X_test_lgbm, "lgbm"))
    test_probas = []
    for model, X_test_curr, _ in all_models:
        test_probas.append(model.predict_proba(X_test_curr))
    return xgb_models, lgbm_models, np.stack(test_probas, axis=1)
...
def build_oof_stack(X_xgb, X_lgbm, y_idx, sample_weight):
    ...
    for fold, (tr_idx, va_idx) in enumerate(cv.split(X_xgb, y_idx)):
...
def main() -> None:
    ...
    _, _, test_model_probas = fit_base_models(
        X_train_xgb,
        X_train_lgbm,
        y_idx,
        sample_weight,
    ...
    proba = 0.7 * meta_proba + 0.3 * base_avg
