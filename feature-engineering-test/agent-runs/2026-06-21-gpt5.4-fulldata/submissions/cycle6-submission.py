...
class SoftVotingEnsemble:
    ...
    def predict_proba(self, X):
        ...
        return np.average(np.stack(probas, axis=0), axis=0, weights=self.weights)
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    redshift_bucket = pd.cut(
        redshift,
        bins=[-np.inf, 0.02, 0.08, 0.2, 0.6, 1.5, 3.0, np.inf],
        labels=["very_low", "low", "mid_low", "mid", "high", "very_high", "extreme"],
    )
...
def build_model() -> SoftVotingEnsemble:
    xgb_a = XGBClassifier(
    ...
    return SoftVotingEnsemble([xgb_a, xgb_b, lgbm], weights=[0.38, 0.24, 0.38])
...
def main() -> None:
    ...
    model = build_model()
    model.models[0].fit(X_train_xgb, y_idx, sample_weight=sample_weight)
    model.models[1].fit(X_train_xgb, y_idx, sample_weight=sample_weight)
    model.models[2].fit(X_train_lgbm, y_idx, sample_weight=sample_weight)

    proba = (
        0.38 * model.models[0].predict_proba(X_test_xgb)
        + 0.24 * model.models[1].predict_proba(X_test_xgb)
        + 0.38 * model.models[2].predict_proba(X_test_lgbm)
    )
