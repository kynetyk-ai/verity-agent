...
def test_layout_render_lists_every_role_as_an_absolute_path() -> None:
    rendered = render_workspace_layout(WORKSPACE_CONTRACT)
    for role in ("data", "context", "tools", "spec", "scratch", "outbox"):
        # roles render as absolute paths (/work/<role>/), not a bare name a model could misread as
        # the read-only root (the outbox/ -> /outbox failure)
        assert f"/work/{role}/" in rendered
...
def test_compose_is_deterministic_and_three_layered() -> None:
    ...
    # as its absolute path), and names the working directory
    assert "working directory is /work" in a
    for role in WORKSPACE_CONTRACT.roles:
        assert f"/work/{role.name}/" in a
